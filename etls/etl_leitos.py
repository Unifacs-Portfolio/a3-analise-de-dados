...
def processar_leitos_repouso_masculino(caminho_arquivo):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="leitos_repouso_masc",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def obter_estatisticas_repouso(caminho_fem, caminho_masc, caminho_ind, caminho_ped):
    ...
    df_par["total_leitos_repouso"] = (
        df_par["leitos_repouso_fem"]
        + df_par["leitos_repouso_masc"]
        + df_par["leitos_repouso_ind"]
        + df_par["leitos_repouso_ped"]
    )
...
def extrair_bloco_infraestrutura(caminhos_arquivos):
    ...
    df_uti_total, df_uti_sus, df_uti_nao_sus = obter_estatisticas_uti(
        caminhos_arquivos["comp_nao_sus"], caminhos_arquivos["comp_sus"]
    )

    df_enf_total, df_enf_sus, df_enf_nao_sus = obter_estatisticas_enfermaria(
        caminhos_arquivos["enf_nao_sus"], caminhos_arquivos["enf_sus"]
    )
    dfs_leitos = [
        df_rep_fem,
        df_rep_masc,
