...
def processar_equip_diagnostico_imagem(caminho):
    df = remove_footer(
        pd.read_csv(
            caminho, encoding="latin1", sep=";", header=4, na_values=["-", " - "]
        )
    )
    ...
    df = df.melt(
        ...
        var_name="ano_mes",
        value_name="equip_diag_imagem_total",
    )
...
def processar_equip_diagnostico_imagem_sus(caminho):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="equip_diag_imagem_sus",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def processar_equip_manutencao_vida(caminho):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="equip_manut_vida_total",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def processar_equip_manutencao_vida_sus(caminho):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="equip_manut_vida_sus",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def processar_equip_metodos_graficos(caminho):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="equip_met_graficos_total",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def processar_equip_metodos_graficos_sus(caminho):
    ...
    df = df.melt(
        id_vars=["id_unidade_federativa", "nome_unidade_federativa"],
        var_name="ano_mes",
        value_name="equip_met_graficos_sus",
    )
    df["ano"] = df["ano_mes"].str.split("/").str[0]
...
def aplicar_logica_estatistica(df, coluna_alvo):
    df_agg = (
        df.groupby(["id_unidade_federativa", "nome_unidade_federativa", "ano"])[
            coluna_alvo
        ]
        .agg(["mean", "median", "std"])
        .reset_index()
    ...
    df_agg[f"desvio_padrao_{coluna_alvo}"] = df_agg[
        f"desvio_padrao_{coluna_alvo}"
    ].round(2)
...
def obter_estatisticas_diagnostico_imagem(caminho_total, caminho_sus):
    ...
    df_par = pd.merge(
        df_tot,
        df_sus,
        on=["id_unidade_federativa", "nome_unidade_federativa", "ano_mes", "ano"],
        how="inner",
    )
    df_par["equip_diag_imagem_nao_sus"] = (
        df_par["equip_diag_imagem_total"] - df_par["equip_diag_imagem_sus"]
    )
...
def obter_estatisticas_totais(caminho_total, caminho_sus):
    ...
    df_par["equip_totais_nao_sus"] = (
        df_par["equip_totais_total"] - df_par["equip_totais_sus"]
    )
...
def obter_estatisticas_manutencao_vida(caminho_total, caminho_sus):
    ...
    df_par["equip_manut_vida_nao_sus"] = (
        df_par["equip_manut_vida_total"] - df_par["equip_manut_vida_sus"]
    )
...
def obter_estatisticas_metodos_graficos(caminho_total, caminho_sus):
    ...
    df_par["equip_met_graficos_nao_sus"] = (
        df_par["equip_met_graficos_total"] - df_par["equip_met_graficos_sus"]
    )
...
def extrair_bloco_equipamentos(caminhos_arquivos):
    ...
    df_diag_total, df_diag_sus, df_diag_nao_sus = obter_estatisticas_diagnostico_imagem(
        caminhos_arquivos["diag_imagem"], caminhos_arquivos["diag_imagem_sus"]
    )
    df_totais_total, df_totais_sus, df_totais_nao_sus = obter_estatisticas_totais(
        caminhos_arquivos["totais"], caminhos_arquivos["totais_sus"]
    )
    df_vida_total, df_vida_sus, df_vida_nao_sus = obter_estatisticas_manutencao_vida(
        caminhos_arquivos["manut_vida"], caminhos_arquivos["manut_vida_sus"]
    )
    df_graf_total, df_graf_sus, df_graf_nao_sus = obter_estatisticas_metodos_graficos(
        caminhos_arquivos["met_graficos"], caminhos_arquivos["met_graficos_sus"]
    )
